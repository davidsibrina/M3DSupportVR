...
public class ObjInteractions : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MainCollider = GameObject.Find("MainCube").GetComponent<Collider>();
    ...
    void Update()
    {
        ...
        float LeftTriggerVal = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger);
        float RightTriggerVal = OVRInput.Get(OVRInput.Axis1D.SecondaryIndexTrigger);
        if (LeftTriggerVal > 0 && RightTriggerVal > 0)
        {
			...
			// David Sibrina Edit
            Scale = 4 * (dist2 - dist1);
            this.gameObject.transform.localScale = new Vector3(StartScale.x + Scale, StartScale.y + Scale, StartScale.z + Scale);
        }
